...
# Agent 节点
# ─────────────────────────────────────────────
SYSTEM_PROMPT = """你是论文重写专家。任务：将一篇学术论文重写为通俗中文文章。

你的工作流程：
1. 先用 search_original 和 read_original_segment 浏览原文，理解整体结构
2. 用 save_outline 保存章节大纲（根据原文长度动态调整章节数：每1-2万字原文对应1章重写，27万字原文应有15-20章）
...
6. 写完所有章节后，用 list_chapters 确认覆盖情况，如果遗漏了重要概念则继续补写
7. 全部完成后回复"写作任务完成"
